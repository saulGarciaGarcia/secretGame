...
var player = (function(player) {
  ...
  player.die  = new Animation(player.sheet, 4, 11, 11);
...
function animate() {
  if (!stop) {
    ...
    if(score != 0 && score % 300 == 0){
      stop = true;
      var r = confirm("Lo lograste! ¿quieres entregar el paquete?");
      if (r == true) {
        $('#credits').hide();
        $('#explanation').show();
        $('#main').hide();
...
$('.start').click(function() {
  ...
  $('.explanation').on('touchstart', function(){
    var url = window.getComputedStyle(document.getElementById("menu"), false).backgroundImage;
    url = url.slice(url.indexOf("/image")+6,-6);
    if(url == 7){
      $('#menu').hide();
      startGame();
    }
    if(url == 10){
    var r = confirm("¿ya te dijo?");
      if (r == true) {
        document.getElementById("menu").style.backgroundImage = "url('imgs/history/image11.png')";
      } else {
        url = 9;
      }
    }
    url++;
    document.getElementById("menu").style.backgroundImage = "url('imgs/history/image"+url+".png')";
    if(url == 12){
      document.getElementById("menu").style.backgroundImage = "url('imgs/history/image11.png')";
    }
  });
